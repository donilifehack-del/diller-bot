...
async def order_select_shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not available:
        await query.edit_message_text("❌ Omborda tovar yo'q.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Orqaga", callback_data="main_menu")]]))
        return ConversationHandler.END
    await query.edit_message_text(f"🏪 Dokon: *{shop['name']}*\n\nQaysi tovarni yuborasiz?",
        reply_markup=kb, parse_mode="Markdown")
...
async def order_enter_qty(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if qty <= 0:
            ...
            return ORDER_ENTER_QTY
        if qty > avail:
            await update.message.reply_text(f"❌ Omborda faqat {avail} ta bor:")
            return ORDER_ENTER_QTY
...
async def order_discount_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if disc_type == "none":
        ...
        await query.edit_message_text(
            ...
            "Qancha pul to'landi? (qarz bo'lsa 0):", parse_mode="Markdown")
    ...
    try:
        ...
        if disc_type == "percent":
            ...
            if val <= 0 or val >= original:
                await update.message.reply_text(f"❌ 0 dan {original:,.0f} gacha kiriting:")
                return ORDER_DISCOUNT_VALUE
            discount_amount = val
...
async def order_enter_paid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if paid < 0:
            ...
            return ORDER_ENTER_PAID
        if paid > total:
            await update.message.reply_text(f"❌ {total:,.0f} so'mdan ko'p bo'lmaydi:")
            return ORDER_ENTER_PAID
        context.user_data["ord_paid"] = paid
        await update.message.reply_text("💬 Izoh kiriting ('-' o'tkazib yuborish):")
        return ORDER_NOTE
    except ValueError:
        await update.message.reply_text("❌ Raqam kiriting:")
        return ORDER_ENTER_PAID
